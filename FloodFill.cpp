...
// Simple Floodfill (repainting single-color connected component)
                    ...
                    std::vector<FFillSegment>* buffer )
    {
        ...
        //用stack代替opencv的源码;没有用到prevl,prevr
        ...
        while (!stack.empty()) {
            ...
            for (k = 0; k < 2; ++k) {
                ...
                for (i = d[1]; i <= d[2]; ++i) {
                    if (rowPtr[i] == val0 and (unsigned) i < image.cols) {
                        int j = i;
                        rowPtr[i] = newVal;

                        //opencv的--j,++i是为了避免rowPtr[j==i]时重复赋值
                        //原本的[L-1,R-1]区间用了--,++变成[L-2,R+2]
                        //push时要[j+1,i+1],确保下一行只比当前行多两个八邻域元素
                        while (j >= 0 and rowPtr[j] == val0) {
                            rowPtr[j] = newVal;
                            j--;
                        }
                        while (i < image.cols and rowPtr[i] == val0) {
                            rowPtr[i] = newVal;
                            i++;
                        }

                        stack.template emplace(
                                FFillSegment{(ushort) (YC + d[0]), (ushort) (j), (ushort) (i), (ushort) L, (ushort) R});
                    }
                }
            }
        }*/
