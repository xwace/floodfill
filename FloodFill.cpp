...
// Simple Floodfill (repainting single-color connected component)
                    ...
                    std::vector<FFillSegment>* buffer )
    {
        ...
        //用stack代替opencv的源码--_8_connectivity确认是否8邻域搜索;没有用到prevl,prevr
        /*std::stack<FFillSegment> stack;
        FFillSegment fs{(ushort) seed.y, (ushort) L, (ushort) R, (ushort) (R + 1), (ushort) R, UP};
        stack.template emplace(fs);
        while (!stack.empty()) {
            auto curLine = stack.top();
            stack.pop();
            ...
            int data[2][3] = {{-1, L - _8_connectivity, R + _8_connectivity},
                              {1,  L - _8_connectivity, R + _8_connectivity}};

            for (k = 0; k < 2; ++k) {
                auto d = data[k];//for循环取上一行和下一行
                _Tp *rowPtr = image.ptr<_Tp>(YC + d[0]);

                if ((unsigned) (YC + d[0]) >= (unsigned) roi.height)
                    continue;

                for (i = d[1]; i <= d[2]; ++i) {
                    if (rowPtr[i] == val0 && (unsigned) i < image.cols) {

                        int j = i;
                        rowPtr[i] = newVal;//(1)

                        //(1)是锚点,(2)从锚点前一位开始扫描(3)锚点后一位开始扫描
                        //--j,++i终止时多移动了一位,最后要补回 [j+1,i-1]
                        while (--j >= 0 && rowPtr[j] == val0)//(2)
                            rowPtr[j] = newVal;

                        while (++i < roi.width && rowPtr[i] == val0)//(3)
                            rowPtr[i] = newVal;

                        stack.template emplace(
                                FFillSegment{(ushort) (YC + d[0]), (ushort) (j + 1), (ushort) (i - 1), (ushort) L,
                                             (ushort) R});
                    }
                }
            }
...
//                            {dir, PR + 1, R + _8_connectivity}
        ...
        namedWindow("floodfillDefault_4nbs",2);
        imshow("floodfillDefault_4nbs",image * 90);
        waitKey();
...
//            mask = _mask.getMat();
    ...
    void run() {
        ...
        img1(1,3) = 1;img1(4,3) = 1;img1(6,3) = 1;
        img1(2,4) = 1;img1(3,4) = 1;img1(4,4) = 1;img1(5,4) = 1;img1(6,4) = 1;
